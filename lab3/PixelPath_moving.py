...
class PixelPath(tk.Tk):
    ...
    def draw_grid(self):
        ...
        for line in self.lines:
            x0, y0, x1, y1 = self.scale_coordinates(line[0], line[1]) + self.scale_coordinates(line[2], line[3])
            if line[-1] == 's':
                self.create_line(x0, y0, x1, y1, new=False)
            if line[-1] == 'd':
                self.dda(*line[:4], new=False)
            if line[-1] == 'b':
                self.bresenham_line(x0, y0, x1, y1, new=False)
            if line[-1] == 'c':
                self.castle_pitway(x0, y0, x1, y1, new=False)
        # repaint circles
        for circle in self.circles:
            self.bresenham_circle(*self.scale_coordinates(circle[0], circle[1]), circle[2] / self.zoom_level, new=False)
    ...
    def zoom_with_scroll(self, event, direction=None):
        if direction == "in" or (not direction and event.delta > 0):
            ...
            return
        self.x_zoom = self.canvas.canvasx(event.x)
        self.y_zoom = self.canvas.canvasy(event.y)
        self.zoom_level *= scale_factor
        self.canvas.scale("all", self.x_zoom, self.y_zoom, scale_factor, scale_factor)
        self.canvas.configure(scrollregion=self.canvas.bbox("all"))
    ...
    def scale_coor(self, x1, x2, y1, y2):
        scale_factor = 1 / self.zoom_level
        self.canvas.scale("all", self.x_zoom, self.y_zoom, scale_factor, scale_factor)
        self.canvas.configure(scrollregion=self.canvas.bbox("all"))
        x1 = self.start_x - self.x_zoom - (x1 + self.start_x - self.x_zoom) * scale_factor
        x2 = self.start_x - self.x_zoom - (x2 + self.start_x - self.x_zoom) * scale_factor
        y1 = self.start_y - self.y_zoom - (y1 + self.start_y - self.y_zoom) * scale_factor
        y2 = self.start_y - self.y_zoom - (y2 + self.start_y - self.y_zoom) * scale_factor
        return x1, y1, x2, y2
        ...
        try:
            ...
            if sign_d_x * d_x:
                for i in np.arange(1, sign_d_x * d_x + 1):
                    ...
                    self.canvas.create_oval(x, y, x + 1, y + 1, fill="black", width=1, tags=["grid", "h"])
            else:
                for i in np.arange(1, abs(d_y) + 1):
                    y = y1 + sign_d_y * i
                    self.canvas.create_oval(x1, y, x1 + 1, y + 1, fill="black", width=1, tags=["grid", "h"])
        except TypeError:
            return
        ...
        try:
            ...
            if abs(d_x) > abs(d_y):
                for i in np.arange(abs(d_x)):
                    ...
                    self.canvas.create_oval(x, y, x + 1, y + 1, fill="black", width=1, tags="grid")
    ...
    def bresenham_line(self, x0=0, y0=0, x1=0, y1=0, new=True):
        ...
        try:
            ...
            y1 += self.start_y
            while True:
                self.canvas.create_oval(x0, y0, x0 + 1, y0 + 1, fill="black", width=1, tags="grid")
                if x0 == x1 and y0 == y1:
                    break
                error2 = 2 * error
                if error2 > -d_y:
    ...
    def bresenham_circle(self, xc=0, yc=0, radius=0, new=True):
        ...
        try:
            ...
            if radius == 0:
                self.canvas.create_oval(xc, yc, xc + 1, yc + 1, fill="black", width=1, tags="grid")
                return
            x = 0
            y = radius
            d = 3 - 2 * radius
    ...
    def plot_circle_points(self, xc, yc, x, y):
        ...
        self.canvas.create_oval(cx + xc + x, cy - (yc + y), cx + xc + x + 1, cy - (yc + y) + 1, fill="black", tags="grid")
        self.canvas.create_oval(cx + xc - x, cy - (yc + y), cx + xc - x + 1, cy - (yc + y) + 1, fill="black", tags="grid")
        self.canvas.create_oval(cx + xc + x, cy - (yc - y), cx + xc + x + 1, cy - (yc - y) + 1, fill="black", tags="grid")
        self.canvas.create_oval(cx + xc - x, cy - (yc - y), cx + xc - x + 1, cy - (yc - y) + 1, fill="black", tags="grid")
        self.canvas.create_oval(cx + xc + y, cy - (yc + x), cx + xc + y + 1, cy - (yc + x) + 1, fill="black", tags="grid")
        self.canvas.create_oval(cx + xc - y, cy - (yc + x), cx + xc - y + 1, cy - (yc + x) + 1, fill="black", tags="grid")
        self.canvas.create_oval(cx + xc + y, cy - (yc - x), cx + xc + y + 1, cy - (yc - x) + 1, fill="black", tags="grid")
        self.canvas.create_oval(cx + xc - y, cy - (yc - x), cx + xc - y + 1, cy - (yc - x) + 1, fill="black", tags="grid")
