...
class PixelPath(tk.Tk):
    ...
    def draw_grid(self):
        ...
        for line in self.lines:
            if line[-1] == 's':
                self.create_line(*line[:4], new=False)
            if line[-1] == 'd':
                self.dda(*line[:4], new=False)
            if line[-1] == 'b':
                self.bresenham_line(*line[:4], new=False)
            if line[-1] == 'c':
                self.castle_pitway(*line[:4], new=False)
        # repaint circles
        for circle in self.circles:
            self.bresenham_circle(*circle, new=False)
        ...
        try:
            ...
            if sign_d_x * d_x:
                ...
                for i in np.arange(1, abs(d_y) + 1):
                    y = y1 + sign_d_y * i
                    self.canvas.create_oval(x1, y, x1 + 1, y + 1, fill="black", width=1, tags="grid")
        except TypeError:
            return
