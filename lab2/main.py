...
def simple_threshold(img, threshold_value=127):
    global tmp_image
...
def apply_simple_threshold():
    global current_image
    if current_image is not None:
        result = simple_threshold(current_image)
        display_image(result)
...
label_brightness.grid(row=4, column=0, padx=(30, 0), pady=1, sticky="w")

brightness_entry = tk.Entry(frame_left, font=("Helvetica", 18), width=5)
brightness_entry.insert(0, "50")
brightness_entry.grid(row=4, column=0, padx=1, pady=1,)
...
btn_adaptive_thresh.grid(row=9, column=0, padx=5, pady=5)

label_simple_thresh = tk.Label(frame_left, text="Saving:", bg="#2c3e50", fg="white", font=("Helvetica", 18))
label_simple_thresh.grid(row=10, column=0, padx=5, pady=5)
